...
class Seq2SeqTrainer(Trainer):
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        if (
            ...
            # self.reissue_pt_warnings(caught_warnings)
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                '''
                inputs['input_ids'].shape = [batch_size, max_source_length] (token_id in each position)
                inputs['attention_mask'].shape = [batch_size, max_source_length] (atten_mask in each position)
                inputs['decoder_input_ids'].shape = [batch_size, max_target_length] (token_id in each position, with SOS)
            ...
            if epoch < int(np.ceil(num_train_epochs)):
                ...
                with open(self.args.output_dir + '/epoch_eval.json', 'a') as epoch_eval:
                    json.dump(metrics, epoch_eval, indent=1)
